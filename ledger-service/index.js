...
// --- PERSISTENCE CONFIG ---
// Save to the mounted volume folder 'data' so it survives restarts
const DB_DIR = path.join(__dirname, 'data');
if (!fs.existsSync(DB_DIR)){
    fs.mkdirSync(DB_DIR);
}
const DB_PATH = path.join(DB_DIR, 'ledger_db.json');
...
app.post('/transaction/update', async (req, res) => {
    ...
    if (db.transactions[transactionId]) {
        ...
        if (status === 'COMPLETED') {
            ...
            if (recipientAcc) {
                recipientAcc.balance += tx.amount;
                console.log(`💰 Credited ${tx.amount} to Recipient ${tx.recipientId}`);
            }
        }
